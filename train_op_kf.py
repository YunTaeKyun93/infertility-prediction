...
def feature_engineering(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ── 신규 피처 ─────────────────────────────────────
    # SHAP 1위 피처(배아_이식률) 구간화 → 비선형 관계 포착
    df["배아_이식률_구간"] = pd.cut(
        df["배아_이식률"],
    ...
    # 나이 × 배아 이식 경과일 → 나이 많을수록 Day5 효과 더 큼
    df["나이_x_이식경과일"] = df["시술 당시 나이"] * df["배아 이식 경과일"]

    # 과거 성공 경험 여부 (임신 횟수 > 0)
    df["과거_성공_경험"] = (df["총 임신 횟수"] > 0).astype(int)

    # 총 생성 배아 중 이식+저장 외 손실된 배아 비율
    df["배아_손실률"] = 1 - (
        (df["이식된 배아 수"] + df["저장된 배아 수"]) / (df["총 생성 배아 수"] + 1)
    )

    # 신선 난자 대비 이식 배아 수 → 전체 프로세스 효율
    df["전체_효율"] = df["이식된 배아 수"] / (df["수집된 신선 난자 수"] + 1)

    # 이식 경과일이 Day 5인지 (성공률 40.4%로 최고)
    df["이식_Day5"] = (df["배아 이식 경과일"] == 5).astype(int)

    print(f"  ✅ 피처 엔지니어링 완료: 기존 11개 + 신규 6개 = 17개")
    return df
...
# ════════════════════════════════════════════════════════
# Clipping 기준값 (상위 99% 기준 — 학습 데이터에서 계산 후 고정)
# 의료 데이터 특성상 이상치도 실제 케이스일 수 있어 제거 대신 상한 고정
# ════════════════════════════════════════════════════════
CLIP_COLS = {
    "이식된 배아 수":         3,    # 의학적 권고 최대치 (ASRM 가이드라인)
    "총 생성 배아 수":        20,   # 상위 99% 기준
    "수집된 신선 난자 수":    20,   # 과자극 증후군 기준 이상
    "저장된 배아 수":         15,   # 상위 99% 기준
    "혼합된 난자 수":         20,   # 상위 99% 기준
    "미세주입된 난자 수":     20,   # 상위 99% 기준
    "미세주입에서 생성된 배아 수": 15,
    "파트너 정자와 혼합된 난자 수": 20,
}

def apply_clipping(df: pd.DataFrame, clip_dict: dict = None) -> pd.DataFrame:
    """
    Clipping — 이상치를 제거하지 않고 상한값으로 고정
    학습/테스트 모두 동일한 기준값 적용
    """
    ...
    for col, upper in clip_dict.items():
        if col in df.columns:
            ...
            if before > upper:
                clipped += 1
    print(f"  ✅ Clipping 완료: {clipped}개 컬럼 상한 적용")
    return df
...
def _base_preprocess(df, is_train):
    ...
    # ── Clipping 적용 (피처 엔지니어링 전에 적용해야 파생 피처에도 반영)
    df = apply_clipping(df)

    df = feature_engineering(df)
    return df, target


def target_encoding(df_train, df_test, y_train, cat_encode_cols):
    """
    타깃 인코딩 — 범주형 컬럼을 해당 카테고리의 임신 성공률로 대체
    K-Fold 내부 누수 방지를 위해 train/test 분리 적용
    """
    ...
    for col in cat_encode_cols:
        if col not in df_train.columns:
            continue
        mean_map = y_train.groupby(df_train[col]).mean()
        global_mean = y_train.mean()
        df_train[f"{col}_te"] = df_train[col].map(mean_map).fillna(global_mean)
        df_test[f"{col}_te"]  = df_test[col].map(mean_map).fillna(global_mean)
...
if __name__ == "__main__":
    ...
    # 타깃 인코딩 적용 (누수 없이 전체 train 기준)
    X_cat, X_cat_test = target_encoding(X_cat, X_cat_test, y, TARGET_ENCODE_COLS)
    X_xgb, X_xgb_test = target_encoding(X_xgb, X_xgb_test, y, TARGET_ENCODE_COLS)
    ...
    submission.to_csv(f"{SAVE_DIR}/submission_v3_kfold_optuna.csv", index=False)
    print("✅ submission_v3_kfold_optuna.csv 저장 완료")
