...
# ════════════════════════════════════════════════════════
# 3. 피처 선택 — LGB 1-Fold로 중요도 빠르게 계산 후 하위 제거
# ════════════════════════════════════════════════════════
def select_features(X, X_test, y, spw, threshold_pct=10):
    """
    LGB 1-Fold로 feature importance 계산
    하위 threshold_pct% 피처 제거
    """
    import lightgbm as lgb
    print(f"\n🔍 피처 선택 중 (하위 {threshold_pct}% 제거)...")
    ...
    drop_cols  = importance[importance <= threshold].index.tolist()
    keep_cols  = importance[importance > threshold].index.tolist()

    print(f"  전체 피처: {len(X.columns)}개")
    print(f"  제거 피처: {len(drop_cols)}개 (중요도 ≤ {threshold:.2f})")
    print(f"  유지 피처: {len(keep_cols)}개")
    print(f"\n  제거된 피처 목록:")
    for col in sorted(drop_cols):
        print(f"    - {col}  (importance: {importance[col]:.1f})")

    # 상위 30개 시각화
    plt.figure(figsize=(10, 8))
    importance.sort_values(ascending=True).tail(30).plot(kind="barh", color="steelblue")
    plt.title("Feature Importance TOP 30 (LightGBM)", fontsize=13)
    plt.tight_layout()
    plt.savefig("feature_importance.png", dpi=150, bbox_inches="tight")
    plt.close()
    print("\n  📊 feature_importance.png 저장")
...
if __name__ == "__main__":
    ...
    submission.to_csv(f"{SAVE_DIR}/submission_fs.csv", index=False)
    print(f"\n✅ submission_fs.csv 저장 완료  (OOF AUC: {opt_auc:.5f})")