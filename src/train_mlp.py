...
class MLP(nn.Module):
    def __init__(self, input_dim, hidden_dims=[512, 512, 256, 256, 128], dropout=0.3):
        ...
        self.res3 = ResidualBlock(hidden_dims[-1], dropout)  # 잔차 블록 추가
...
def train_mlp_fold(X_tr, y_tr, X_val, y_val, X_test, input_dim, pos_weight):
    model     = MLP(input_dim, hidden_dims=[512, 512, 256, 256, 128], dropout=0.3).to(DEVICE)
    ...
    swa_start     = 50  # 150 epoch 이후부터 SWA 적용
    ...
    patience, no_improve = 50, 0  # patience 증가

    for epoch in range(300):  # epoch 증가
        model.train()
        for xb, yb in loader:
    ...
    if epoch >= swa_start:
        torch.optim.swa_utils.update_bn(
            DataLoader(TensorDataset(X_tr_t), batch_size=2048),
            swa_model,
            device=DEVICE #type:ignore
        )
        swa_model.eval()
        with torch.no_grad():
            swa_val  = torch.sigmoid(swa_model(X_val_t)).cpu().numpy()
            swa_test = torch.sigmoid(swa_model(X_tst_t)).cpu().numpy()
        swa_auc = roc_auc_score(y_val, swa_val)

        # SWA가 더 좋으면 SWA 사용
        if swa_auc > best_auc:
            best_auc       = swa_auc
            best_val_pred  = swa_val
            best_test_pred = swa_test
...
if __name__ == "__main__":
    ...
    pd.DataFrame({"ID": ids, "probability": tst_pred}).to_csv(
        os.path.join(SAVE_DIR, "submission_mlp.csv"), index=False
    )
